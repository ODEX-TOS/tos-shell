...
class Dock extends Dash.Dash {
    _init() {
        ...
        this.set_opacity(Math.round(settings.get_int('dock-icons-opacity') / 100 * 255));
        this._background.set_opacity(Math.round(settings.get_int('dock-background-opacity') / 100 * 255));
    ...
    _on_dock_hover() {
        if (!settings.get_boolean('dock-always-show') && !this._dashContainer.get_hover() && !this.keep_dock_shown) {
            this.auto_hide_dock_timeout = GLib.timeout_add(GLib.PRIORITY_DEFAULT, settings.get_int('dock-autohide-delay'), () => {
    ...
    _hide_dock() {
        ...
        this.ease({
            duration: settings.get_int('dock-hide-dock-duration'),
    ...
    _show_dock() {
        ...
        this.dock_animated = true;
        this.ease({
            duration: settings.get_int('dock-show-dock-duration'),
...
class Extension {
    ...
    _dock_refresh() {
        ...
        if (settings.get_boolean('dock-always-show') || this.dock.is_visible()) {
        ...
        this.dock.show();
        if (!settings.get_boolean('dock-always-show') && !this.dock._dashContainer.get_hover()) {
    ...
    _on_screen_border_box_hover() {
        if (settings.get_boolean('dock-always-show')) {
            return;
        }
        ...
        if (!Main.overview.visible && !Main.sessionMode.isLocked) {
            ...
            this.toggle_dock_hover_timeout = GLib.timeout_add(GLib.PRIORITY_DEFAULT, settings.get_int('dock-toggle-delay'), () => {
                if (settings.get_boolean('dock-show-in-full-screen') || !global.display.get_focus_window() || !global.display.get_focus_window().is_fullscreen()) {
                    if (this.screen_border_box.get_hover()) {
    ...
    _on_settings_changed() {
        this.dock._background.set_opacity(Math.round(settings.get_int('dock-background-opacity') / 100 * 255));
        this.dock.set_opacity(Math.round(settings.get_int('dock-icons-opacity') / 100 * 255));
        this._dock_refresh();
    }
    ...
    constructor() {
        settings = ExtensionUtils.getSettings('org.gnome.shell.extensions.tos')

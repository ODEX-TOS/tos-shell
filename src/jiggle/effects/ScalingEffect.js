...
const Cursor = Me.imports.src.jiggle.cursor;
const { Effects } = Me.imports.src.jiggle.effects;
...
const ScalingIcon = GObject.registerClass(
  ...
  class ScalingIcon extends St.Icon {
    _init(params = {}) {
      ...
      this.custom_cursor = Gio.icon_new_for_string(
        Me.path + "/src/jiggle/icons/jiggle-cursor.png"
      );
        ...
        switch (this.animation) {
          ...
          case Effects.AnimationDirections.GROW:
            ...
            if (++this.animation_current_frame < this.animation_frames.length) {
              this.current_size =
                this.animation_frames[this.animation_current_frame];
            } else {
    ...
    start() {
      ...
      this.animation_frames = Effects.animate(
        this.current_size,
        this.system_cursor_size * this.cursor_scale_factor,
        this.growth_speed * this.speed_factor,
        Effects.Transitions.easeOutQuad
      );
      if (this.animation_current_frame >= this.animation_frames.length) {
    ...
    stop() {
      ...
      this.animation_frames = Effects.animate(
        this.current_size,
        this.system_cursor_size,
        this.shrink_speed * this.speed_factor,
        Effects.Transitions.easeInQuad
      );
      if (this.animation_current_frame >= this.animation_frames.length) {
        this.animation_current_frame = this.animation_frames.length - 1;
      }
    ...
    update() {
      this.use_system = true;
      this.hide_cursor = true;
      this.growth_speed = Math.max(0.1, 0.35);
      this.shrink_speed = Math.max(0.1, 0.25);
      if (!this.use_system) {
        this.gicon = this.custom_cursor;
